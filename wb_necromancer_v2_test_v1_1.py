...
diff --git a/wb_necromancer_v2_test_v1_1.py b/wb_necromancer_v2_test_v1_1.py
new file mode 100644
index 0000000000000000000000000000000000000000..00349ac05e685f8f701bbe4959f651119f0126b7
--- /dev/null
+++ b/wb_necromancer_v2_test_v1_1.py
@@ -0,0 +1,4607 @@
+# -*- coding: utf-8 -*-
+"""
...
+        market_phone_counts[safe_str(r.get("market_phone") or "UNKNOWN")] += 1
+        market_type_counts[safe_str(r.get("market_type") or "UNKNOWN")] += 1
+
...
+        f = safe_str(flag).upper()
+        if any(x in f for x in ("PRICE", "PRIC", "ДЕМП")):
+            return "price"
+        if any(x in f for x in ("KEY", "SEO", "QUERY", "TERM")):
+            return "seo"
+        if any(x in f for x in ("CONTENT", "PHOTO", "DESC", "TITLE", "MEDIA", "VIDEO")):
+            return "content"
+        if any(x in f for x in ("RATING", "REVIEW", "FEEDBACK", "KARMA")):
+            return "social"
+        return "other"
+
+    def _badge_verdict(vv: str) -> str:
+        v = safe_str(vv)
+        cls = {
+            "REVIVE_FAST": "v-fast",
+            "REVIVE_REWORK": "v-rework",
+            "CLONE_NEW_CARD": "v-clone",
+            "DROP": "v-drop",
+        }.get(v, "v-unk")
+        return f"<span class='badge {cls}'>{_html.escape(v)}</span>"
+
+    def _render_items(items: List[str], limit: int = 8) -> str:
+        arr = [safe_str(x).strip() for x in items if safe_str(x).strip()]
+        if not arr:
+            return "<div class='muted'>Нет данных</div>"
+        return "<ul>" + "".join([f"<li>{_html.escape(x)}</li>" for x in arr[:limit]]) + "</ul>"
+
+    def _render_counter_tags(counter_obj: Any, css_prefix: str, limit: int = 10) -> str:
+        if not counter_obj:
+            return "<div class='muted'>Нет данных</div>"
+        items = []
+        for k, v in counter_obj.most_common(limit):
+            items.append(
+                f"<span class='tag {css_prefix}' title='{_html.escape(safe_str(k))}'>"
+                f"{_html.escape(safe_str(k))} <span class='cnt'>{int(v)}</span></span>"
+            )
...
+        flags_raw = [x.strip() for x in safe_str(r.get("risk_flags") or "").split(",") if x.strip()]
+        cat_set = sorted(set(_flag_category(x) for x in flags_raw))
+        cat_attr = ",".join(cat_set)
+
+        flags_html = " ".join([f"<span class='tag c-{_flag_category(x)}'>{_html.escape(x)}</span>" for x in flags_raw])
+        if not flags_html:
+            flags_html = "<span class='muted'>—</span>"
+
+        backlog_lines = [x.strip("• ").strip() for x in safe_str(r.get("backlog") or "").splitlines() if x.strip()]
+        rationale_lines = [x.strip() for x in safe_str(r.get("rationale") or "").split("|") if x.strip()]
+        backlog_text = "\n".join(backlog_lines)
...
+    <div class="mono muted">{_html.escape(safe_str(r.get("vendor_code")))}</div>
+  </td>
+  <td>
+    <a href="{_html.escape(safe_str(r.get("url")))}" target="_blank" rel="noreferrer">{_html.escape(safe_str(r.get("name")))}</a>
+    <div class="muted">{_html.escape(safe_str(r.get("phone_model")))}</div>
+  </td>
+  <td>{_badge_verdict(verdict)}</td>
+  <td>
...
+
+    css = """
+:root{--bg:#0b1020;--panel:#0f1220;--line:#24283b;--text:#e6e6e6;--muted:#9aa4bf;--accent:#9ad1ff}
+*{box-sizing:border-box}
+body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;margin:0;background:var(--bg);color:var(--text)}
+header{position:sticky;top:0;background:var(--panel);border-bottom:1px solid var(--line);padding:12px 16px;z-index:10}
...
+.card .k{color:var(--muted);font-size:11px}.card .v{font-size:20px;font-weight:700}
+.controls{display:flex;gap:10px;flex-wrap:wrap;margin:10px 0 14px}
+input,select{background:var(--bg);color:var(--text);border:1px solid var(--line);border-radius:8px;padding:8px 10px}
+.chk{font-size:12px;color:var(--muted);display:flex;align-items:center;gap:4px}
+.btn{background:var(--bg);color:var(--text);border:1px solid var(--line);border-radius:8px;padding:8px 10px;cursor:pointer}
+.summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:10px;margin:12px 0 14px}
+.panel{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:12px}
+.panel h2{margin:0 0 8px 0;font-size:14px}
+.panel ul{margin:0;padding-left:18px}
+table{width:100%;border-collapse:collapse}
+th,td{border-bottom:1px solid var(--line);padding:8px 10px;vertical-align:top;font-size:12px}
+th{position:sticky;top:62px;background:var(--panel);text-align:left}
+a{color:var(--accent)}
+.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
+.muted{color:var(--muted)}
+pre{white-space:pre-wrap;margin:0;font-size:12px}
+.badge{display:inline-block;padding:3px 8px;border-radius:999px;border:1px solid var(--line);font-size:11px}
+.v-fast{background:#12301c;border-color:#1f6b3b}.v-rework{background:#33290d;border-color:#8d6a1a}.v-clone{background:#1d2546;border-color:#4152a3}.v-drop{background:#451818;border-color:#8f3434}
+.tag{display:inline-block;padding:2px 6px;border:1px solid var(--line);border-radius:999px;margin:0 4px 4px 0;font-size:11px}
+.c-price{background:#2a1b1b}.c-seo{background:#1b243a}.c-content{background:#1c2a1d}.c-social{background:#2b2213}
+.t-top{background:#111a30}.cnt{opacity:.8;margin-left:4px}
+"""
+
+    js = """
...
+  const verdict=(document.getElementById('verdict').value||'ALL');
...
+    }
+    if(ok && verdict!=='ALL') ok = (r.dataset.verdict===verdict);
+    const cats = (r.dataset.cats||'').split(',').filter(Boolean);
+    const needsCat = fPrice || fSeo || fContent || fSocial;
+    if(ok && needsCat){
+      const hit = (fPrice && cats.includes('price'))
+        || (fSeo && cats.includes('seo'))
+        || (fContent && cats.includes('content'))
+        || (fSocial && cats.includes('social'));
+      ok = hit;
+    }
...
+}
+['q','verdict','fPrice','fSeo','fContent','fSocial'].forEach(id=>{
+  document.getElementById(id).addEventListener(id==='q'?'input':'change', applyFilters);
+});
+const openAll = document.getElementById('openAll');
+const closeAll = document.getElementById('closeAll');
+if(openAll){ openAll.addEventListener('click', ()=>document.querySelectorAll('details').forEach(x=>x.open=true)); }
+if(closeAll){ closeAll.addEventListener('click', ()=>document.querySelectorAll('details').forEach(x=>x.open=false)); }
+applyFilters();
+"""
+
...
+  <div class='card'><div class='k'>REVIVE_FAST</div><div class='v'>{verdict_counts.get('REVIVE_FAST',0)}</div></div>
...
+<div class='summary'>
+  <section class='panel'>
+    <h2>Топ risk flags</h2>
+    {_render_counter_tags(risk_flags_counter, 't-top', limit=14)}
+  </section>
+  <section class='panel'>
+    <h2>Топ backlog задач</h2>
+    {_render_counter_tags(backlog_counter, 't-top', limit=14)}
+  </section>
+  <section class='panel'>
+    <h2>Доказательная база</h2>
+    <ul>
+      <li>SKU в scope: {len(rows)}</li>
+      <li>market_phone: {_html.escape(json.dumps(dict(market_phone_counts), ensure_ascii=False))}</li>
+      <li>market_type: {_html.escape(json.dumps(dict(market_type_counts), ensure_ascii=False))}</li>
+      <li>Источник: decisions.jsonl + cluster_verdicts.jsonl + intent.jsonl</li>
+    </ul>
+  </section>
+</div>"""
+
...
+  <div class='mono muted'>run_id={_html.escape(run_id)} • shown=<span id='shown'>0</span> / {len(rows)} • source=decisions.jsonl + cluster_verdicts.jsonl + intent.jsonl</div>
+  <div class='controls'>
...
+    </select>
+    <label class='chk'><input type='checkbox' id='fPrice'>price</label>
+    <label class='chk'><input type='checkbox' id='fSeo'>seo</label>
+    <label class='chk'><input type='checkbox' id='fContent'>content</label>
+    <label class='chk'><input type='checkbox' id='fSocial'>social</label>
+    <button id='openAll' class='btn' type='button'>Развернуть всё</button>
+    <button id='closeAll' class='btn' type='button'>Свернуть всё</button>
+  </div>
+</header>
+<div class='container'>
+{cards_html}
