...
## 10) Hotfix Stage M: deterministic summary by default (anti-garbage)

### Проблема
- В summary-блоках Stage M появлялись шумные LLM-формулировки и мусорные числовые утверждения.
- Это нарушало требование воспроизводимости: текст мог конфликтовать с фактами из JSONL.

### Исправление
- Свободный LLM narrative в Stage M выключен по умолчанию в рендере.
- Добавлен детерминированный `run_summary`, рассчитанный только из артефактов JSONL:
  - `counts_by_verdict`
  - `market_phone` / `market_type`
  - `top_risk_flags`
  - `top_backlog_tasks`
  - `top_backlog_by_tag` и `top_backlog_by_prio`
- HTML summary-панели рендерятся из `run_summary` (детерминированно).
- LLM (если включён через `--use-llm-m` или alias `--use-llm-exec-summary`) ограничен до JSON-схемы:
  - `{ "notes": [str], "plan_7d": [str] }`
  - любые цифры в тексте запрещены валидатором; при нарушении ответ отбрасывается.
- XLSX `SUMMARY` теперь также строится на детерминированных полях `run_summary` (LLM-заметки добавляются только как нечисловые при успешной валидации).
